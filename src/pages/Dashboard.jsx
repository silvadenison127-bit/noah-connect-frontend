...
import {
  Calendar, AlertTriangle, AlertCircle, Info, ArrowRight, Sparkles
} from "lucide-react";
import api from "../services/api";
import VisaoGeralMetrics from "../components/VisaoGeralMetrics";
import CardPedidosOracaoResumo from "../components/CardPedidosOracaoResumo";
  ...
  return (
    <div className="relative w-24 h-24 shrink-0">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-base font-bold text-white">{percentual !== null ? `${percentual}%` : "--"}</span>
...
function CardSaudeIgreja({ saude, carregando }) {
  ...
  return (
    <div className="bg-[#0F0F1E] rounded-xl border border-white/10 shadow-sm p-3 h-full">
      <h3 className="font-semibold text-white text-sm mb-0.5">Saúde da Igreja</h3>
      <p className="text-[11px] text-slate-500 mb-2">Baseado em dados reais da plataforma</p>
      {carregando ? (
...
function CardRecomendacoes({ recomendacoes, carregando }) {
  ...
  return (
    <div className="bg-[#0F0F1E] rounded-xl border border-white/10 shadow-sm p-3">
      <h3 className="font-semibold text-white text-sm mb-2">Ações Sugeridas pela IA</h3>
      {carregando ? (
        <p className="text-sm text-slate-500">Carregando...</p>
  ...
  return (
    <div className="bg-[#0F0F1E] rounded-xl border border-white/10 shadow-sm p-3 h-full">
      <div className="flex items-center gap-2 mb-0.5">
        ...
        <h3 className="font-semibold text-white text-sm">Previsões da IA</h3>
      </div>
      <p className="text-[11px] text-slate-500 mb-2">Baseado em dados históricos</p>
      {carregando ? (
        <p className="text-sm text-slate-500">Carregando...</p>
      ) : (
        <div className="grid grid-cols-2 gap-2">
          {previsoes?.map((p) => (
            <div key={p.metrica} className="bg-white/[0.03] border border-white/5 rounded-lg p-2">
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2 bg-[#0F0F1E] border border-white/10 rounded-xl px-3 py-1.5 text-sm text-slate-300 shadow-sm">
          <Calendar size={14} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-2">
        ...
        <div className="bg-[#0F0F1E] rounded-xl border border-white/10 shadow-sm p-3">
          <div className="flex items-center justify-between mb-2">
            <h3 className="font-semibold text-white text-sm">Pedidos de Oração Recentes</h3>
            <button className="text-xs text-violet-400 font-medium">Ver todos</button>
          </div>
          {carregando ? (
            <p className="text-sm text-slate-500">Carregando...</p>
                ...
                return (
                  <div key={p.id} className="flex items-center gap-2.5">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[11px] text-slate-500 truncate">{p.anonimo ? "Anônimo" : p.nome_solicitante}</p>
                    </div>
                    <span className="text-[11px] text-slate-500 shrink-0">{quando}</span>
                  </div>
                );
              })}
            </div>
        ...
        <div className="bg-[#0F0F1E] rounded-xl border border-white/10 shadow-sm p-3">
          <div className="flex items-center justify-between mb-2">
            <h3 className="font-semibold text-white text-sm">Últimos Cadastros</h3>
            <button className="text-xs text-violet-400 font-medium">Ver todos</button>
          </div>
          {carregando ? (
            <p className="text-sm text-slate-500">Carregando...</p>
                ...
                return (
                  <div key={m.id} className="flex items-center gap-2.5">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[11px] text-slate-500 capitalize">{m.tipo}</p>
                    </div>
                    <span className="text-[11px] text-slate-500 shrink-0">{quando}</span>
                  </div>
                );
        ...
        <div className="bg-[#0F0F1E] rounded-xl border border-white/10 shadow-sm p-3 self-start">
          ...
          <div className="flex items-center gap-3">
            <div className="w-24 h-24 relative shrink-0">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={dadosCelulas} dataKey="valor" innerRadius={32} outerRadius={46} paddingAngle={2}>
                    {dadosCelulas.map((_, i) => (<Cell key={i} fill={CORES_CELULA[i % CORES_CELULA.length]} stroke="none" />))}
                  </Pie>
                </PieChart>
              </ResponsiveContainer>
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-base font-bold text-white">{totalMembros.toLocaleString("pt-BR")}</span>
